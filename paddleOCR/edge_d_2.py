...
def process_image(ocr_engine, image_path, is_file=True, visualize=False):
    ...
    try:
        ...
        try:
            ...
            # If OCR returned no results, try the original label image
            if result is None or len(result) == 0 or len(result[0]) == 0:
                print("No OCR results on preprocessed image, trying original label...")
                original_rgb = cv2.cvtColor(label_img, cv2.COLOR_BGR2RGB)
                result = ocr_engine.ocr(original_rgb, cls=True)
        except Exception as e:
            print(f"OCR error: {str(e)}. Trying original label image...")
            # Fallback to original image if preprocessing caused issues
            original_rgb = cv2.cvtColor(label_img, cv2.COLOR_BGR2RGB)
            result = ocr_engine.ocr(original_rgb, cls=True)
        ...
        import traceback
        traceback.print_exc()
        return img if 'img' in locals() else None, None, None, False
        return img, None, None, False
...
def detect_label(image, visualize=False):
    """
    Detect shipping label in the image - specifically optimized for delivery boxes
    with white/light colored shipping labels
    ...
    # For labels on purple/colored backgrounds (like in the example images)
    # Try color-based segmentation first
    # Convert to HSV color space
    hsv = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)
    
    # Define range for white/light color (shipping label)
    # Adjust these values based on your specific images
    lower_white = np.array([0, 0, 150])  # Low saturation, high value
    upper_white = np.array([180, 30, 255])  # All hues, low saturation, high value
    
    # Create a mask for white/light regions
    ...
    if contours:
        ...
        # Filter for rectangle-like contours with reasonable size
        for contour in contours[:5]:  # Check the 5 largest contours
            ...
            # Typical shipping labels have aspect ratio between 0.8 and 2.5
            if 0.8 <= aspect_ratio <= 2.5:
                # This is likely our shipping label
                if visualize:
                    debug_img = image.copy()
                ...
                # Crop the label with a small margin
                margin = 5
    ...
    # If no suitable contour was found with the first method, try another approach
    ...
    # Invert if needed (we want the label to be white)
    ...
    # Look for rectangular shapes using edge detection
    ...
    if contours:
        ...
        for contour in contours[:5]:
            ...
            # Get rotated rectangle (handles angled labels better)
            ...
            # Check if it's a reasonable label shape
            if 0.8 <= aspect_ratio <= 2.5:
                if visualize:
                    debug_img = image.copy()
                    cv2.drawContours(debug_img, [box], 0, (0, 255, 0), 2)
                    cv2.imshow("Detected Label - Method 2", debug_img)
                    cv2.waitKey(0)
                
                # Get a proper crop using the rotated rectangle
    ...
    # Third approach - try simple color thresholding for white regions
    print("Second method failed, trying simple thresholding...")
    ...
    if contours:
        ...
        # Take the largest contour that has a reasonable size
        for contour in contours[:3]:
            area = cv2.contourArea(contour)
            
            # Label should be at least 5% of the image
            if area >= (image.shape[0] * image.shape[1]) * 0.05:
                x, y, w, h = cv2.boundingRect(contour)
                
                # Check aspect ratio
                aspect_ratio = w / float(h)
                if 0.5 <= aspect_ratio <= 3.0:  # Be more lenient here
                    if visualize:
                        debug_img = image.copy()
                        cv2.rectangle(debug_img, (x, y), (x+w, y+h), (0, 255, 0), 2)
                        cv2.imshow("Detected Label - Method 3", debug_img)
                        cv2.waitKey(0)
    ...
    # Last resort: Return the central portion of the image
    print("All detection methods failed. Using center portion of image...")
    h, w = image.shape[:2]
    # Crop central 60% of the image
    x1 = int(w * 0.2)
    y1 = int(h * 0.2)
    x2 = int(w * 0.8)
    y2 = int(h * 0.8)
...
def preprocess_label(label_image, visualize=False):
    ...
    if label_image is None or label_image.size == 0:
        raise ValueError("Invalid label image for preprocessing")
    ...
    # Invert if necessary (ensure text is black on white background for OCR)
    if np.mean(binary) < 127:
        binary = cv2.bitwise_not(binary)
    
    if visualize:
        plt.figure(figsize=(15, 10))
        plt.subplot(231), plt.imshow(cv2.cvtColor(label_image, cv2.COLOR_BGR2RGB)), plt.title('Original Label')
        plt.subplot(232), plt.imshow(gray, cmap='gray'), plt.title('Grayscale')
        plt.subplot(233), plt.imshow(enhanced, cmap='gray'), plt.title('CLAHE Enhanced')
        plt.subplot(234), plt.imshow(sharpened, cmap='gray'), plt.title('Sharpened')
        plt.subplot(235), plt.imshow(denoised, cmap='gray'), plt.title('Denoised')
        plt.subplot(236), plt.imshow(binary, cmap='gray'), plt.title('Binary')
        plt.tight_layout()
        plt.show()
    ...
    # Return both the preprocessed binary image and the original label image
    # This gives OCR options to try both
    return label_image  # Return original for better OCR with PaddleOCR
